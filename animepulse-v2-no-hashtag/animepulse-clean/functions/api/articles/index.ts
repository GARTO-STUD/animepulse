...
export const onRequestGet: PagesFunction<Env> = async ({ request, env }) => {
  ...
  if (type === 'trending') {
    ...
    if (news) {
      ...
      const published = articles.filter((a: Record<string, unknown>) => !a.status || a.status === 'published');
  ...
  try {
    ...
    articles = articles.filter((a: Record<string, unknown>) => !a.status || a.status === 'published');
