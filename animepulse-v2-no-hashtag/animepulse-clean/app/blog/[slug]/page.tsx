...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const url = `${process.env.NEXT_PUBLIC_APP_URL || 'https://animepulse.online'}/blog/${slug}`;
  return {
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <Link href="/blog" className="inline-flex items-center gap-1.5 text-[#64748b] hover:text-[#0f172a] text-sm mb-8 transition-colors">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-3xl sm:text-4xl font-black text-[#0f172a] leading-tight mb-4">{post.title}</h1>
          <p className="text-[#64748b] text-lg mb-5">{post.summary}</p>
        ...
        {/* Cover */}
        <div className="relative w-full h-72 rounded-2xl overflow-hidden border border-[#e2e8f4] mb-10">
        ...
        <div className="space-y-4 mb-12">
          {post.content.split('\n').map((para, i) => {
            ...
            if (para.startsWith('## ')) return (
              <h2 key={i} className="text-[#0f172a] text-2xl font-black mt-10 mb-3 flex items-center gap-2">
                <div className="w-1 h-6 bg-[#e85d04] rounded-full flex-shrink-0" />
                {para.replace('## ', '')}
              </h2>
            );
            if (para.startsWith('### ')) return <h3 key={i} className="text-[#0f172a] text-xl font-bold mt-6 mb-2">{para.replace('### ', '')}</h3>;
            return <p key={i} className="text-[#64748b] leading-relaxed text-[1.05rem]">{para}</p>;
          })}
        </div>

        {/* Tags */}
        <div className="flex flex-wrap gap-2 pt-8 border-t border-[#e2e8f4] mb-14">
          <Tag className="w-4 h-4 text-[#64748b] mt-0.5" />
          {post.tags.map(tag => (
            <span key={tag} className="text-xs bg-white border border-[#e2e8f4] text-[#64748b] px-3 py-1 rounded-full">
              {tag}
            </span>
          ))}
        </div>
            ...
            {otherPosts.map(([slug, p]) => (
              <Link key={slug} href={`/blog/${slug}`}
                ...
                <div className="p-4">
                  ...
                  <h3 className="text-[#0f172a] text-sm font-bold line-clamp-2 group-hover:text-[#e85d04] transition-colors">{p.title}</h3>
                </div>
              </Link>
            ))}
          </div>
