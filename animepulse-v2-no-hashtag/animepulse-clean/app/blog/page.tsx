...
export default function BlogPage() {
  ...
  return (
    <div className="min-h-screen">
      ...
      {/* Header */}
      <div className="bg-white border-b border-[#e2e8f4] py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-3 mb-2">
            <BookOpen className="w-7 h-7 text-[#e85d04]" />
            <h1 className="text-3xl font-black text-[#0f172a]">AnimePulse Blog</h1>
          </div>
          <p className="text-[#64748b]">In-depth guides, top lists, and expert analysis from the world of anime.</p>

          {/* Category filters */}
          <div className="flex flex-wrap gap-2 mt-6">
            {CATEGORIES.map(cat => (
              <span key={cat}
                className="px-4 py-1.5 rounded-full text-xs font-bold bg-[#f8f9fc] border border-[#e2e8f4] text-[#64748b] cursor-default hover:text-[#e85d04] hover:border-[#e85d04]/30 transition-colors">
                {cat}
              </span>
            ))}
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <Link href={`/blog/${featured.slug}`}
          className="block relative rounded-2xl overflow-hidden mb-12 border border-[#e2e8f4] group hover:border-[#e85d04]/30 transition-colors">
          ...
          <div className="relative p-8 sm:p-12 max-w-3xl">
            <div className="flex items-center gap-3 mb-5">
              <span className="bg-[#e85d04] text-[#0f172a] text-xs font-black px-3 py-1 rounded-full uppercase tracking-wide">Featured</span>
              <span className="text-[#e85d04] text-xs font-bold">{featured.category}</span>
            </div>
            <h2 className="text-3xl sm:text-4xl font-black text-[#0f172a] mb-4 group-hover:text-[#f48c06] transition-colors leading-tight">
              {featured.title}
            </h2>
            <p className="text-[#64748b] text-lg mb-6 max-w-2xl">{featured.summary}</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {rest.map(post => (
            <Link key={post.slug} href={`/blog/${post.slug}`}
              className="bg-white border border-[#e2e8f4] rounded-xl overflow-hidden group hover:border-[#e85d04]/30 transition-colors">
              <div className="relative h-48 overflow-hidden">
                <img src={post.image} alt={post.title}
                  className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300 opacity-80" />
                <div className="absolute top-3 left-3">
                  <span className="bg-[#f8f9fc]/90 border border-[#e2e8f4] text-[#e85d04] text-xs font-bold px-2.5 py-1 rounded-full">
                    {post.category}
                  </span>
                </div>
              </div>
              <div className="p-5">
                ...
                <p className="text-[#64748b] text-sm line-clamp-2 mb-4">{post.summary}</p>
                <div className="flex items-center justify-between text-xs text-[#64748b] pt-3 border-t border-[#e2e8f4]">
