...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  const title   = (article.seoTitle as string) || article.title as string;
  const summary = (article.metaDescription as string) || article.summary as string;
  const image   = (article.imageUrl as string) || `${APP_URL}/og-image.jpg`;
  const url     = `${APP_URL}/news/${id}`;
  const tags    = (article.tags as string[]) || [];
  const kws     = (article.keywords as string[]) || tags;
...
export default async function NewsDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const articleSchema = article ? {
    ...
    keywords: ((article.keywords as string[]) || (article.tags as string[]) || []).join(', '),
  } : null;
  ...
  return (
    <>
      ...
      <NewsDetailClient id={id} initialArticle={article} />
    </>
  );
}
