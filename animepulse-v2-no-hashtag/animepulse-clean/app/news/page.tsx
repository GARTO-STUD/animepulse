...
export default function NewsPage() {
  ...
  const loadArticles = useCallback(async (tab: SourceKey, p: number, tag: string | null) => {
    setLoading(true);
    try {
  ...
  useEffect(() => {
    loadArticles(activeTab, page, selectedTag);
  ...
  return (
    <div className="min-h-screen bg-[#f8f9fc]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        {loading ? (
          <ArticleCardSkeletonGrid count={12} />
        ) : articles.length === 0 ? (
          <div className="text-center py-24">
            <span className="text-6xl mb-4 block">✦</span>
            <p className="text-[#0f172a] font-bold text-lg mb-2">No articles found</p>
            <p className="text-[#64748b] text-sm">Try a different filter or check back soon</p>
          </div>
        ) : (
          <>
                  ...
                  <button
                    ...
                    className={`text-xs px-3 py-1.5 rounded-full border transition-colors ${
                      selectedTag === tag
                        ...
                        : 'bg-white border-[#e2e8f4] text-[#64748b] hover:border-[#e85d04]/30 hover:text-[#0f172a]'
                    }`}
                  >
                    {tag}
                  </button>
