...
function Stars({ score }: { score: number | null }) {
  ...
  return (
    <div className="flex gap-0.5">
      {[1,2,3,4,5].map(i => (
        <Star key={i} className={`w-4 h-4 ${i <= s ? 'fill-yellow-400 text-yellow-400' : 'text-[#cbd5e1]'}`} />
      ))}
    </div>
  );
}
...
export default function ReviewsPage() {
  ...
  const scoredAnime = anime.filter(a => a.score);
  const avgScore = scoredAnime.length
    ? (scoredAnime.reduce((s, a) => s + (a.score || 0), 0) / scoredAnime.length).toFixed(1)
    : '—';
      ...
      <div className="bg-white border-b border-[#e2e8f4] py-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-3xl font-black text-[#0f172a] section-title">Anime Reviews</h1>
          </div>
          <p className="text-[#64748b]">Community ratings &amp; scores from MyAnimeList &mdash; the world&apos;s largest anime database.</p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {[
            ...
            <div key={s.label} className="bg-white border border-[#e2e8f4] rounded-xl p-5 text-center">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
              ...
              <button
                ...
                className={`px-4 py-1.5 rounded-xl text-xs font-bold transition-all ${
                  filter === f
                    ...
                    : 'bg-white border border-[#e2e8f4] text-[#64748b] hover:text-[#0f172a]'
                }`}
              >
                {f}
              </button>
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="bg-white border border-[#e2e8f4] text-[#64748b] text-sm rounded-lg px-3 py-1.5 outline-none"
            >
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[...Array(8)].map((_, i) => (
              <div key={i} className="bg-white rounded-xl h-36 animate-pulse border border-[#e2e8f4]" />
            ))}
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {filtered.map((a) => (
              <Link key={a.mal_id} href={`/anime/${a.mal_id}`} className="anime-card flex gap-4 bg-white border border-[#e2e8f4] rounded-xl p-4 group block">
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="text-[#0f172a] font-bold text-sm line-clamp-1 group-hover:text-[#e85d04] transition-colors mb-1">
                    {a.title}
                  </h3>
                  <Stars score={a.score} />
                  <p className="text-[#64748b] text-xs mt-1 line-clamp-2">{a.synopsis ? (a.synopsis.length > 100 ? a.synopsis.slice(0, 100) + '…' : a.synopsis) : '—'}</p>
                  <div className="flex items-center gap-3 mt-2 text-xs">
                    ...
                    <span className="text-[#64748b]">{a.genres[0]?.name || '—'}</span>
                  </div>
                </div>
                {a.rank && (
                  <div className="flex-shrink-0 text-right">
                    <div className="text-[#64748b] text-xs">Rank</div>
