...
/** Trailer button for grid cards — stops link propagation */
function GridTrailerButton({ anime }: { anime: Anime }) {
  ...
  return (
    <>
      <button
        ...
        className="absolute bottom-2 left-2 z-10 flex items-center gap-1 bg-red-600 hover:bg-red-500 text-[#0f172a] text-[10px] font-bold px-2 py-1 rounded-lg transition-all shadow-lg"
  ...
  return (
    <div className="relative">
      <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); setOpen(!open); }}
        className="text-xs text-[#64748b] hover:text-[#0f172a] transition-colors px-2 py-1 rounded hover:bg-[#f1f5f9]">
        Share
      </button>
      {open && (
        <div className="absolute bottom-8 right-0 z-50 bg-white border border-[#e2e8f4] rounded-xl shadow-xl overflow-hidden min-w-[150px]"
          onClick={(e) => { e.preventDefault(); e.stopPropagation(); }}>
          <a href={twitterUrl} target="_blank" rel="noopener noreferrer"
            className="flex items-center gap-2 px-4 py-2.5 text-sm text-[#64748b] hover:text-[#0f172a] hover:bg-[#f1f5f9]">
          ...
          <button onClick={copyLink}
            className="w-full flex items-center gap-2 px-4 py-2.5 text-sm text-[#64748b] hover:text-[#0f172a] hover:bg-[#f1f5f9]">
    ...
    <div className="min-h-screen">
      ...
      <div className="bg-white border-b border-[#e2e8f4] py-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h1 className="text-3xl font-black text-[#0f172a] mb-1">Trending Anime</h1>
          <p className="text-[#64748b]">Live rankings from MyAnimeList. Updated daily.</p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          {TABS.map((t, i) => (
            <button key={t} onClick={() => setTab(i)}
              className={`px-5 py-2.5 rounded-xl text-sm font-bold transition-all ${
                tab === i
                  ? 'bg-[#e85d04] text-[#0f172a]'
                  : 'bg-white border border-[#e2e8f4] text-[#64748b] hover:text-[#0f172a] hover:border-[#e85d04]/30'
              }`}>
              {t}
            </button>
          ))}
        ...
        {loading ? (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {[...Array(10)].map((_, i) => (
              <div key={i} className="bg-white rounded-xl h-64 animate-pulse border border-[#e2e8f4]" />
            ))}
          </div>
        ) : (
          <>
            {/* Featured */}
            {featured && (
              <div className="relative rounded-2xl overflow-hidden mb-8 border border-[#e2e8f4]">
                ...
                <div className="relative flex flex-col md:flex-row gap-6 p-8">
                  ...
                  <div className="flex-1">
                    <span className="inline-block bg-[#e85d04] text-[#0f172a] text-xs font-bold px-2 py-0.5 rounded mb-3">#1 {TABS[tab]}</span>
                    <h2 className="text-2xl font-black text-[#0f172a] mb-2">{featured.title}</h2>
                    <p className="text-[#64748b] text-sm mb-4 line-clamp-3 max-w-2xl">{featured.synopsis}</p>
                    <div className="flex flex-wrap gap-4 text-sm items-center">
                      {featured.score && (
                        <span className="text-yellow-400 font-bold">
                          ★ {featured.score}
                          {featured.scored_by && <span className="text-[#64748b] font-normal ml-1">({fmt(featured.scored_by)})</span>}
                        </span>
                      )}
                      <span className="text-[#64748b]">{fmt(featured.members)} members</span>
                      <span className="text-[#64748b]">{featured.genres.slice(0, 3).map(g => g.name).join(' · ')}</span>
                      <span className="text-[#64748b]">{featured.episodes ? `${featured.episodes} eps` : featured.status}</span>
                      <Link href={`/anime/${featured.mal_id}`} className="text-[#e85d04] hover:underline font-semibold text-sm">View Details →</Link>
                      <ShareButton
                    ...
                    {featured.trailer?.youtube_id && (
                      <div className="mt-5 max-w-xs">
                        <TrailerPreviewCard youtubeId={featured.trailer.youtube_id} title={featured.title} />
                      </div>
                    )}
                  </div>
                </div>
                ...
                <Link key={anime.mal_id} href={`/anime/${anime.mal_id}`} className="group block">
                  <div className="relative rounded-xl overflow-hidden bg-white border border-[#e2e8f4]">
                    <div className="relative h-56">
                      ...
                      <span className="absolute top-2 left-2 w-6 h-6 bg-[#e85d04] rounded-full flex items-center justify-center text-[#0f172a] text-xs font-black z-10">
                        {i + 2}
                      </span>
                      {anime.score && (
                      ...
                      <GridTrailerButton anime={anime} />
                    </div>
                    <div className="p-3">
                      <h3 className="text-[#0f172a] text-xs font-bold line-clamp-2">{anime.title}</h3>
                      <p className="text-[#64748b] text-xs mt-1">{anime.genres[0]?.name || '—'}</p>
                      <div className="flex items-center justify-between mt-1">
                        <p className="text-[#64748b] text-xs">{anime.episodes ? `${anime.episodes} eps` : 'Ongoing'}</p>
                        <ShareButton
