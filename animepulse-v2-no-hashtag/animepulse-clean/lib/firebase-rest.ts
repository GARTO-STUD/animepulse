...
export interface ArticleVersion {
  ...
  editedAt: string;
  editedBy?: string; // future: admin user id
}
...
export interface Article {
  ...
  keywords?: string[];
  /** Full edit history — max 10 versions kept */
...
export function fsVal(v: unknown): unknown {
  ...
  if ('arrayValue' in o)   return Array.isArray((o.arrayValue as { values?: unknown[] })?.values) ? ((o.arrayValue as { values: unknown[] }).values).map(fsVal) : [];
  if ('mapValue' in o) {
...
export function verifyAdminPassword(provided: string | null | undefined): boolean {
  // IMPORTANT: Only use server-side env vars here (no NEXT_PUBLIC_ prefix)
  // This function runs in edge runtime only — never on the client
