  ...
  try {
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      ...
      if (response.status === 429) {
        ...
        console.warn(`⏳ Gemini 429 rate limit, retrying in ${wait}ms (attempt ${attempt + 1}/3)`);
        await sleep(wait);
        continue;
      }
      break;
    ...
    if (!response || !response.ok) {
      throw new Error(`Gemini API error: ${response?.status ?? 'no response'}`);
    }
