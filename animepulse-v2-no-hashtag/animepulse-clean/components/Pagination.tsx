  ...
  return (
    <div className={`flex items-center justify-center gap-1 ${className}`}>
      <button
        ...
        disabled={page === 1}
        className="p-2 rounded-xl bg-white border border-[#e2e8f4] text-[#64748b] hover:text-[#0f172a] hover:border-[#e85d04]/30 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
      >
      ...
      {range.map((n, i) =>
        n === '...' ? (
          <span key={`ellipsis-${i}`} className="px-2 text-[#64748b] text-sm">…</span>
        ) : (
          <button
            key={n}
