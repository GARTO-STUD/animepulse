...
async function runAutoPilot(saJson: string, geminiKey: string) {
  ...
  const tr = (await fetch('https://api.jikan.moe/v4/top/anime?filter=airing&limit=8').then(r=>r.ok?r.json():{data:[]}).catch(()=>({data:[]})) ) as { data?: { title: string }[] };
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const [statusRes, articles] = await Promise.all([
      fetch(`https://firestore.googleapis.com/v1/projects/${pid}/databases/(default)/documents/meta/autopilot-status`,{headers:{Authorization:`Bearer ${token}`}}).then(r=>r.ok?r.json().then((d:unknown)=> {
        const data = d as { fields?: Record<string, unknown> };
        return data.fields?Object.fromEntries(Object.entries(data.fields).map(([k,v])=>[k,fsVal(v)])):null;
      }):null),
