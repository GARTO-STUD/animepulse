 ...
 * Prioritizes Cloudflare KV for performance, falls back to Firestore REST API.
 */
...
// @ts-expect-error - getRequestContext is available at runtime in Cloudflare
...
async function getFirebaseToken(saJson: string): Promise<string> {
  ...
  // Use Web Crypto for Edge compatibility
  ...
  const signData = new TextEncoder().encode(`${h}.${p}`);
  const sigBuffer = await crypto.subtle.sign('RSASSA-PKCS1-v1_5', key, signData);
  const sig = b64(String.fromCharCode(...new Uint8Array(sigBuffer)));
...
export async function GET(req: NextRequest) {
  ...
  let env: { ANIMEPULSE_KV?: { get: (k: string) => Promise<string|null> }; FIREBASE_SERVICE_ACCOUNT_KEY?: string } = {};
  try {
    // Attempt to get Cloudflare context if running on Pages
    env = (getRequestContext() as { env: typeof env }).env || {};
  } catch {
    // Fallback to process.env for local development
    env = process.env;
  }

  // 1. Try Cloudflare KV first (set by Auto-Pilot worker)
  if (env.ANIMEPULSE_KV) {
    if (type === 'trending') {
      const trending = await env.ANIMEPULSE_KV.get('trending');
      if (trending) return new Response(trending, { headers: { ...CORS, 'Content-Type': 'application/json', 'X-Source': 'KV' } });
    } else if (!source && !tag) {
      const news = await env.ANIMEPULSE_KV.get('news');
      if (news) {
        const articles = JSON.parse(news);
        return NextResponse.json({ articles: articles.slice(0, limit), total: articles.length }, { headers: { ...CORS, 'X-Source': 'KV' } });
      }
    }
  }

  // 2. Fallback to Firestore REST API
  const saJson = env.FIREBASE_SERVICE_ACCOUNT_KEY || process.env.FIREBASE_SERVICE_ACCOUNT_KEY;
  if (!saJson) {
    return NextResponse.json({ articles: [], total: 0, warning: 'Firebase key not configured' }, { headers: CORS });
  }
  ...
  try {
    ...
    if (type === 'trending') {
      ...
      if (r.ok) {
        ...
        const result = d.fields ? Object.fromEntries(Object.entries(d.fields).map(([k, v]) => [k, fsVal(v)])) : { anime: [], updatedAt: null };
    ...
    if (source) articles = articles.filter((a: any) => a.sourceType === source);
    if (tag) articles = articles.filter((a: any) => Array.isArray(a.tags) && a.tags.includes(tag));
